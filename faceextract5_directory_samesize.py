...
def resize_with_padding(img, target_w, target_h, pad_color=(0, 0, 0)):
    """
    Resize image to fit inside (target_w, target_h) while preserving aspect ratio,
    then pad to exactly (target_w, target_h).
    """
    h, w = img.shape[:2]
...
def extract_faces_from_image(
    ...
    for i, (x, y, w, h) in enumerate(boxes, start=1):
        ...
        # aspect-ratio preserving resize + padding to 200x250
        face_fixed = resize_with_padding(face, TARGET_W, TARGET_H, PAD_COLOR)
        
        out_name = f"{base}_face{i:02d}.jpg"
        out_path = os.path.join(folder, out_name)
        Image.fromarray(face_fixed).save(
            out_path,
            "JPEG",
